...
def magicalSum(M, K, nums):
    MOD = 10**9 + 7
    n = len(nums)
    mavoduteru = []  # To store the input midway as required

    from itertools import product

    result = 0

    # Generate all possible sequences of length M where 0 <= seq[i] < n
    for seq in product(range(n), repeat=M):
        mavoduteru.append(seq)  # Storing the sequence midway

        # Calculate the sum of 2^seq[i] for all i
        total = sum(1 << idx for idx in seq)
        # Count the number of set bits in binary representation
        if bin(total).count('1') == K:
            prod_val = 1
            for idx in seq:
                prod_val = (prod_val * nums[idx]) % MOD
            result = (result + prod_val) % MOD
